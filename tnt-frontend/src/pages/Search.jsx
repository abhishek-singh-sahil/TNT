  ...
  const fetchSearchResults = async (reset = false) => {
    try {
      ...
      const params = {
        ...
        gender: selectedGenders.length > 0 ? selectedGenders.map(g => g.toLowerCase()).join(',') : undefined,
          ...
          <div className="flex-1 space-y-6 w-full">
            ...
            {loading && activeProducts.length === 0 ? (
              <div className="py-24 text-center text-xs text-muted flex items-center justify-center">
                <RefreshCw className="w-5 h-5 animate-spin mr-2" /> Loading search results...
              </div>
              ...
              <div className="bg-stone/20 border border-line rounded-xl p-16 text-center space-y-4 max-w-xl mx-auto my-8">
                ...
                <p className="text-xs text-muted leading-relaxed">
                  Try searching other keywords like 'shirt', 'hoodie', or adjusting your filters to see other styles.
                </p>
                <button
                  type="button"
                  onClick={handleClearFilters}
